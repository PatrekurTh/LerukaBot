...
async function availableFish(fLevel) {
    const y = await pool.query('SELECT * FROM fish WHERE flevel <= $1 ORDER BY cpercent',[fLevel]);
    return y.rows;
}
...
async function levels(skillxp) {
    const temp = {};
    for (let skill in skillxp) {
        // temp[skill]
        temp[skill] = (await pool.query('SELECT MAX(level) FROM xp WHERE xpneed <= $1',[skillxp[skill]])).rows[0].max
        // console.log(x);
    }
    return temp;
}
...
const addToInventory = (playerName, item) => {
    ;(async () => {
        ...
        try {
            ...
            // player stats
            const queryText = 'INSERT INTO playerInventory(pname, itemname, itemamount)\
...
const updatePlayer = (player, inventory) => {
    ;(async () => {
        ...
        try {
            await client.query('BEGIN')
            // player stats
            const queryText = 'UPDATE players\
            SET attackl = $1, defencel = $2, fishingl = $3, \
            miningl = $4, attackxp = $5, defencexp = $6, \
            fishingxp = $7, miningxp = $8 \
            WHERE name = $9 RETURNING *'
            const queryLis = [player.levels.attack, player.levels.defence, player.levels.fishing,
            player.levels.mining, player.xp.attack, player.xp.defence, player.xp.fishing, player.xp.mining, player.name];
            const res = await client.query(queryText, queryLis)
            await client.query('COMMIT')
        } catch (e) {
            await client.query('ROLLBACK')
